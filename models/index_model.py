...
def get_bike_rental(conn,id_bike):
    return pd.read_sql('''
...
def to_rent(conn,date,bike,client):
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO bike_rental (date_issue,id_bike,id_client) VALUES (:date,:bike,:client);
    ''', {"date": date,"bike":bike,"client":client})
    conn.commit()
    return True

#index.html
def get_bicycle_list(conn,m,b,t,y,table):
    if table is None:
        table='get_tabl4'
